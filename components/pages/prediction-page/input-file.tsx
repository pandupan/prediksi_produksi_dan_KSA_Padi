...
import { UploadCloud, File as FileIcon, Loader2, AlertCircle, BarChart3, LineChart as LineChartIcon, Table2, AreaChart, MapPin } from 'lucide-react';
import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, TooltipProps } from 'recharts';
import { ValueType, NameType, Payload } from 'recharts/types/component/DefaultTooltipContent';

// --- Import Data GeoJSON ---
import { tasikmalayaGeoJson } from '@/lib/tasikmalaya-geojson'; // Sesuaikan path jika perlu
...
const getPhaseColor = (phase: number | null): string => {
    ...
    switch (phase) {
        case 5: return '#A1887F'; case 1: return '#AED581'; case 2: return '#66BB6A';
        case 3.1: return '#FFEE58'; case 3.2: return '#FFCA28'; case 3.3: return '#FFA726';
        case 4: return '#FF7043'; case 13: return '#8D6E63'; case 6: return '#B71C1C';
...
const CustomTooltip = ({ active, payload, label }: MyCustomTooltipProps) => {
    if (active && payload && payload.length) {
        return ( <Card className="p-2 text-sm shadow-lg"><CardHeader className="p-1 font-bold border-b mb-1">{typeof label === 'string' || typeof label === 'number' ? formatKsaDate(String(label)) : String(label)}</CardHeader><CardContent className="p-1">{payload.map((pld: Payload<ValueType, NameType>) => ( <div key={pld.dataKey as React.Key} className="flex items-center"><div style={{ backgroundColor: pld.color as string }} className="w-2.5 h-2.5 rounded-full mr-2 shrink-0"></div><span className="flex-1 truncate">{pld.dataKey as string}: </span><span className="font-semibold ml-2">{yAxisValueMap[String(pld.value)] || pld.value}</span></div>))}</CardContent></Card> );
    }
    return null;
};
...
const InputFile = () => {
    ...
    // --- STATE untuk Peta dengan tombol konfirmasi ---
    ...
    const generatePredictions = (aggData: AggregatedData[], aggCols: string[]): { predictions: PredictedData[], columns: string[] } => {
        ...
        aggData.forEach(row => {
            ...
            newColumns.slice(1).forEach(monthKey => {
                ...
                const nextPhase = (currentIndex === -1 || currentIndex === faseOrder.length - 1) ? faseOrder[0] : faseOrder[currentIndex + 1];
    ...
    const processFile = (file: File) => {
        ...
        setPredictedData(null); setPendingSelectedKecamatan([]); setConfirmedSelectedKecamatan([]);
        setPendingSelectedKecamatanPrediksi([]); setConfirmedSelectedKecamatanPrediksi([]);
        setAllKecamatan([]); setFileName(file.name); 
        setPendingMapMonth(''); setConfirmedMapMonth('');
        // ... sisa fungsi processFile tidak berubah
        ...
        reader.onload = (e: ProgressEvent<FileReader>) => {
            try {
                ...
                if (jsonData.length > 1) {
                    ...
                    const rows: ExcelData[] = jsonData.slice(1).map((row: any[]) => headers.reduce((acc: ExcelData, header: string, index: number) => { if (header) acc[header] = row[index]; return acc; }, {} as ExcelData));
    ...
    const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => { if (event.target.files?.[0]) processFile(event.target.files[0]); };
    const handleDragOver = (event: React.DragEvent<HTMLLabelElement>) => event.preventDefault();
    const handleDrop = (event: React.DragEvent<HTMLLabelElement>) => { event.preventDefault(); if (event.dataTransfer.files?.[0]) processFile(event.dataTransfer.files[0]); };
    ...
    const handleConfirmMapMonth = () => {
        setConfirmedMapMonth(pendingMapMonth);
    };
    ...
    return (
        <section className="w-full max-w-7xl mx-auto p-4 md:p-6 space-y-6">
            ...
            <Card>
                ...
                <CardContent>
                    <label htmlFor="file-upload" className="flex flex-col items-center justify-center w-full h-64 border-2 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors" onDragOver={handleDragOver} onDrop={handleDrop}>
                        <div className="flex flex-col items-center justify-center pt-5 pb-6"><UploadCloud className="w-10 h-10 mb-4 text-gray-500" /><p className="mb-2 text-sm text-gray-500"><span className="font-semibold">Klik untuk unggah</span> atau seret dan lepas</p><p className="text-xs text-gray-500">Membutuhkan kolom 'id segmen' dan 'subsegmen'</p></div>
            ...
            {/* Sisa Tampilan Data (Tabel & Chart) */}
            {data && ( <Card><CardHeader><CardTitle className="flex items-center"><FileIcon className="w-5 h-5 mr-2"/>Pratinjau Data Mentah</CardTitle><CardDescription>Menampilkan semua baris mentah dari file yang diunggah.</CardDescription></CardHeader><CardContent><ScrollArea className="w-full whitespace-nowrap rounded-md border"><div className="h-[450px] relative"><Table><TableHeader className="sticky top-0 z-10 bg-background"><TableRow>{columns.map((col) => ( <TableHead key={col} className="font-semibold">{formatKsaDate(col)}</TableHead> ))}</TableRow></TableHeader><TableBody>{data.map((row, rowIndex) => ( <TableRow key={rowIndex}>{columns.map((col) => ( <TableCell key={col}>{row[col]}</TableCell> ))}</TableRow>))}</TableBody></Table></div><ScrollBar orientation="horizontal" /></ScrollArea></CardContent></Card> )}
            {aggregatedData && aggregatedData.length > 0 && ( <Card><CardHeader><CardTitle className="flex items-center"><BarChart3 className="w-5 h-5 mr-2"/>Analisis Agregat per Kecamatan</CardTitle><CardDescription>Tabel ini menampilkan nilai modus (fase tanam dominan) per bulan untuk setiap kecamatan.</CardDescription></CardHeader><CardContent><ScrollArea className="w-full whitespace-nowrap rounded-md border"><div className="h-[450px] relative"><Table><TableHeader className="sticky top-0 z-10 bg-background"><TableRow>{aggregatedColumns.map((col) => ( <TableHead key={col} className="font-semibold capitalize">{formatKsaDate(col)}</TableHead> ))}</TableRow></TableHeader><TableBody>{aggregatedData.map((row, rowIndex) => ( <TableRow key={rowIndex}>{aggregatedColumns.map((col) => ( <TableCell key={col}>{row[col]}</TableCell>))}</TableRow>))}</TableBody></Table></div><ScrollBar orientation="horizontal" /></ScrollArea></CardContent></Card> )}
            {aggregatedData && aggregatedData.length > 0 && ( <Card><CardHeader><CardTitle className="flex items-center"><LineChartIcon className="w-5 h-5 mr-2"/>Visualisasi Tren Fase Tanam</CardTitle><CardDescription>Grafik tren nilai modus fase tanam dari waktu ke waktu per kecamatan. Pilih kecamatan di bawah ini untuk ditampilkan.</CardDescription></CardHeader><CardContent><div className="mb-4"><Label htmlFor="kecamatan-select">Pilih Kecamatan (Data Aktual)</Label><Select open={isSelectOpen} onOpenChange={setIsSelectOpen} onValueChange={() => {}}><SelectTrigger id="kecamatan-select"><SelectValue placeholder="Pilih kecamatan...">{pendingSelectedKecamatan.length === 0 && "Tidak ada yang dipilih"}{pendingSelectedKecamatan.length === 1 && pendingSelectedKecamatan[0]}{pendingSelectedKecamatan.length === allKecamatan.length && "Semua Kecamatan"}{pendingSelectedKecamatan.length > 1 && pendingSelectedKecamatan.length < allKecamatan.length && `${pendingSelectedKecamatan.length} Kecamatan terpilih`}</SelectValue></SelectTrigger><SelectContent className="max-h-60 overflow-y-auto">{allKecamatan.map((kecamatan) => ( <div key={kecamatan} className="relative flex cursor-default select-none items-center rounded-sm py-1.5 pl-8 pr-2 text-sm outline-none focus:bg-accent focus:text-accent-foreground data-[disabled]:pointer-events-none data-[disabled]:opacity-50" onMouseDown={(e) => e.preventDefault()} onClick={() => handlePendingKecamatanSelect(kecamatan)}><Checkbox id={`checkbox-${kecamatan}`} checked={pendingSelectedKecamatan.includes(kecamatan)} className="absolute left-2 top-1/2 -translate-y-1/2" /><Label htmlFor={`checkbox-${kecamatan}`} className="flex-1 cursor-pointer"> {kecamatan} </Label></div>))}<div className="p-2 border-t"><Button onClick={handleConfirmSelection} className="w-full"> Konfirmasi Pilihan </Button></div></SelectContent></Select></div><div className="h-[400px] w-full relative"><ResponsiveContainer width="100%" height="100%"><LineChart data={chartData} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="name" stroke="hsl(var(--muted-foreground))" fontSize={12} /><YAxis stroke="hsl(var(--muted-foreground))" fontSize={12} domain={[0, 'dataMax + 1']} tickFormatter={(value) => yAxisValueMap[String(value)] || String(value)} /><Tooltip content={<CustomTooltip />} /><Legend />{confirmedSelectedKecamatan.map((kecamatan, index) => ( <Line key={kecamatan} type="monotone" dataKey={kecamatan} stroke={lineColors[index % lineColors.length]} strokeWidth={2} activeDot={{ r: 6 }} /> ))}</LineChart></ResponsiveContainer>{confirmedSelectedKecamatan.length === 0 && ( <div className="absolute inset-0 flex items-center justify-center bg-background/50 backdrop-blur-sm"><p className="text-muted-foreground font-medium">Pilih setidaknya satu kecamatan untuk menampilkan data.</p></div> )}</div></CardContent></Card> )}
            {predictedData && combinedTableData.data.length > 0 && ( <Card><CardHeader><CardTitle className="flex items-center"><Table2 className="w-5 h-5 mr-2"/>Tabel Prediksi Fase Tanam (Aktual + 12 Bulan Prediksi)</CardTitle><CardDescription>Tabel ini menampilkan data aktual yang digabungkan dengan 12 bulan hasil prediksi fase tanam.</CardDescription></CardHeader><CardContent><ScrollArea className="w-full whitespace-nowrap rounded-md border"><div className="h-[450px] relative"><Table><TableHeader className="sticky top-0 z-10 bg-background"><TableRow>{combinedTableData.columns.map((col) => ( <TableHead key={col} className="font-semibold capitalize">{formatKsaDate(col)}</TableHead> ))}</TableRow></TableHeader><TableBody>{combinedTableData.data.map((row, rowIndex) => ( <TableRow key={rowIndex}>{combinedTableData.columns.map((col) => ( <TableCell key={col}>{row[col]}</TableCell>))}</TableRow>))}</TableBody></Table></div><ScrollBar orientation="horizontal" /></ScrollArea></CardContent></Card> )}
            {predictedData && predictedData.length > 0 && ( <Card><CardHeader><CardTitle className="flex items-center"><AreaChart className="w-5 h-5 mr-2"/>Visualisasi Prediksi Tren Fase Tanam</CardTitle><CardDescription>Grafik ini menampilkan tren prediksi fase tanam untuk 12 bulan ke depan.</CardDescription></CardHeader><CardContent><div className="mb-4"><Label htmlFor="kecamatan-select-prediksi">Pilih Kecamatan (Data Prediksi)</Label><Select open={isSelectOpenPrediksi} onOpenChange={setIsSelectOpenPrediksi} onValueChange={() => {}}><SelectTrigger id="kecamatan-select-prediksi"><SelectValue placeholder="Pilih kecamatan...">{pendingSelectedKecamatanPrediksi.length === 0 && "Tidak ada yang dipilih"}{pendingSelectedKecamatanPrediksi.length === 1 && pendingSelectedKecamatanPrediksi[0]}{pendingSelectedKecamatanPrediksi.length === allKecamatan.length && "Semua Kecamatan"}{pendingSelectedKecamatanPrediksi.length > 1 && pendingSelectedKecamatanPrediksi.length < allKecamatan.length && `${pendingSelectedKecamatanPrediksi.length} Kecamatan terpilih`}</SelectValue></SelectTrigger><SelectContent className="max-h-60 overflow-y-auto">{allKecamatan.map((kecamatan) => ( <div key={kecamatan} className="relative flex cursor-default select-none items-center rounded-sm py-1.5 pl-8 pr-2 text-sm outline-none focus:bg-accent focus:text-accent-foreground data-[disabled]:pointer-events-none data-[disabled]:opacity-50" onMouseDown={(e) => e.preventDefault()} onClick={() => handlePendingKecamatanSelectPrediksi(kecamatan)}><Checkbox id={`checkbox-prediksi-${kecamatan}`} checked={pendingSelectedKecamatanPrediksi.includes(kecamatan)} className="absolute left-2 top-1/2 -translate-y-1/2" /><Label htmlFor={`checkbox-prediksi-${kecamatan}`} className="flex-1 cursor-pointer"> {kecamatan} </Label></div>))}<div className="p-2 border-t"><Button onClick={handleConfirmSelectionPrediksi} className="w-full"> Konfirmasi Pilihan </Button></div></SelectContent></Select></div><div className="h-[400px] w-full relative"><ResponsiveContainer width="100%" height="100%"><LineChart data={predictedChartData} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="name" stroke="hsl(var(--muted-foreground))" fontSize={12} /><YAxis stroke="hsl(var(--muted-foreground))" fontSize={12} domain={[0, 'dataMax + 1']} tickFormatter={(value) => yAxisValueMap[String(value)] || String(value)} /><Tooltip content={<CustomTooltip />} /><Legend />{confirmedSelectedKecamatanPrediksi.map((kecamatan, index) => ( <Line key={kecamatan} type="monotone" dataKey={kecamatan} stroke={lineColors[index % lineColors.length]} strokeWidth={2} activeDot={{ r: 6 }} /> ))}</LineChart></ResponsiveContainer>{confirmedSelectedKecamatanPrediksi.length === 0 && ( <div className="absolute inset-0 flex items-center justify-center bg-background/50 backdrop-blur-sm"><p className="text-muted-foreground font-medium">Pilih setidaknya satu kecamatan untuk menampilkan data.</p></div> )}</div></CardContent></Card> )}

            {/* --- KARTU PETA (DIPINDAHKAN KE BAWAH) --- */}
            {combinedTableData.data.length > 0 && (
                <Card>
                    <CardHeader>
                        <CardTitle className="flex items-center"><MapPin className="w-5 h-5 mr-2"/>Peta Sebaran Fase Tanam</CardTitle>
                        <CardDescription>Peta ini memvisualisasikan fase tanam dominan di setiap kecamatan. Pilih bulan lalu klik "Terapkan" untuk melihat perubahan.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <div className="flex items-center gap-2 mb-4">
                            <div className="flex-grow">
                                <Label htmlFor="month-select-map">Pilih Bulan</Label>
                                <Select value={pendingMapMonth} onValueChange={setPendingMapMonth}>
                        ...
                        <TasikMap 
                            geoJsonData={tasikmalayaGeoJson}
                            data={combinedTableData.data}
                            selectedMonth={confirmedMapMonth}
                            phaseColorMapping={getPhaseColor}
                            phaseNameMapping={yAxisValueMap}
                        />
                    </CardContent>
                </Card>
            )}
